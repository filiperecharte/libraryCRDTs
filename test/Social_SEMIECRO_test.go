...
func TestSocialSEMIECRO(t *testing.T) {
	...
	property := func(operations []int, numReplicas int, numOperations int) bool {
		...
		for i := 0; i < numReplicas; i++ {
			replicas[i] = datatypes.NewSocialCRDTECROReplica(strconv.Itoa(i), channels, numOperations-operations[i])
		}
		...
		for i := range replicas {
			...
			go func(r *replica.Replica, operations int) {
				...
				for j := 0; j < operations; j++ {
					...
					case 0:
						OPType = "accept"
						OPValue := datatypes.SocialOpValue{From: rand.Intn(5), To: rand.Intn(5)}
					...
					case 1:
						...
						user := rand.Intn(len(q.(datatypes.SocialState).Friends))
						friends := q.(datatypes.SocialState).Friends[user].ToSlice()
						...
						OPValue := datatypes.SocialOpValue{From: user, To: friend}
					...
					case 3:
						...
						user := rand.Intn(len(q.(datatypes.SocialState).Requesters))
						requests := q.(datatypes.SocialState).Requesters[user].ToSlice()
						...
						OPValue := datatypes.SocialOpValue{From: user, To: requested}
						r.Prepare(OPType, OPValue)
					}
				}
		...
		for i := 1; i < numReplicas; i++ {
			...
			if !datatypes.CompareSocialStates(st.(datatypes.SocialState), stt.(datatypes.SocialState)) {
				for i := 0; i < numReplicas; i++ {
					st, _ := replicas[i].Crdt.Query()
